...
void foreground(char *command, char *newHomeDirectory, char *previousDirectory, char command_greater_than_2[])
{
        ...
        if (e_flag)
        {
            ...
            if (args[check])
            {
                if (args[check][0] == '~' || args[check][0] == '.')
                {
                    ...
                    search_directory(args[check], file_name, currentDirectory, newHomeDirectory, d_flag, f_flag, e_flag, &file_count, &dir_count, something);
            }
            else
    ...
    else if (strcmp(args[0], "ping") == 0)
    {
        ...
        int pid_given = atoi(args[1]);
        int signal_number_given = atoi(args[2]);

        ping(pid_given, signal_number_given);
    }
    else if (strcmp(args[0], "neonate") == 0)
    {
        if (args[1])
        {
            if (strcmp(args[1], "-n") == 0)
            {
                ...
                if (args[2])
                    neonate_time = atoi(args[2]);

                if (neonate_time > 0)
                    neonate(neonate_time);
            }
            else
            {
                printf("Format wrong for neonate\n");
                return;
            }
        }
        else
        {
            printf("Wrong neonate input\n");
            return;
        }
    }
    ...
    {
        char temporary_alias[MAX_SIZE_OF_NAMES] = {0};
        int k = 1;
        while (k != i)
        {
            strcat(temporary_alias, args[k]);
            strcat(temporary_alias, " ");
            k++;
        }

        create_alias(temporary_alias);
        return;
    }
    // {
    //     if (args[2])
    ...
    //     else
    //     {
    //         foreground_pid = new_foreground_fork;

    //         add_process(command, new_foreground_fork, "Running");
